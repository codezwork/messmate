...
const Auth = {
  ...
  showLoginScreen(role) {
    ...
    document.getElementById("login-title").innerText =
      role === "member" ? "Member Login" : "Owner Login";
  ...
  showRegistrationForm(userEmail) {
    ...
    if (this.state.currentRole === "member") {
      document
        .getElementById("step-register-member")
...
function switchOwnerTab(tabName) {
  ...
  // Hide all tabs
  document
    .querySelectorAll("#dashboard-content .mobile-tab-content")
    .forEach((el) => {
...
function getCurrentUid() {
  return (
    localStorage.getItem("messmate_uid") ||
    (auth.currentUser ? auth.currentUser.uid : Auth.state.mockUid)
  );
}
...
function completeMemberRegistration() {
  ...
  if (!uid) {
    showAuthError("Authentication lost. Please try logging in again.");
    return;
  }
  ...
  if (!name || !email || !occupation || !whatsapp || !location || !genderEl) {
...
function completeOwnerRegistration() {
  ...
  if (!canteenName || !ownerName || !price) {
    alert("Fill all fields");
    return;
  }
...
function displayDiscoveryList(canteens) {
  ...
  if (canteens.length === 0) {
    list.innerHTML = `<div class="col-span-full text-center">No canteens found.</div>`;
    return;
  }

  list.innerHTML = canteens
    .map(
      (c) => `
...
function filterDiscoveryList(query) {
  ...
  const filtered = window.allDiscoveryCanteens.filter(
    (c) =>
      (c.canteenName || "").toLowerCase().includes(lower) ||
      (c.locationString || "").toLowerCase().includes(lower),
  );
...
function joinMess(ownerUid) {
  ...
  if (!user || !user.uid) {
    alert(
      "Session Error: We could not verify your account. Please log out and log back in, or do a Hard Refresh (Ctrl+Shift+R).",
    );
    return;
  }
    ...
    db.collection("users")
      ...
      .then((doc) => {
        if (doc.exists) {
          ...
          if (messNameEl)
            messNameEl.innerText = ownerData.canteenName || "My Mess";
...
function initMemberCalendar(user, owner) {
  ...
  if (!grid) return;
  grid.innerHTML =
    "<p class='col-span-7 text-center text-slate-400 py-8'>Loading calendar...</p>";
  ...
  if (user.holidays && user.holidays.length) {
    user.holidays.forEach((h) => {
      if (!prefs[h]) prefs[h] = "Off";
    });
  }
  ...
  Promise.all(requests)
    .then((snapshots) => {
      ...
      for (let i = 1; i <= daysInMonth; i++) {
        ...
        if (typeof userPref === "string")
          userPref = { lunch: userPref, dinner: userPref };
        userPref = userPref || {};
        ...
        const getPillStyle = (type) => {
          if (type === "NonVeg")
            return "bg-rose-100 text-rose-700 hover:bg-rose-200 border border-rose-200";
          if (type === "Fast")
            return "bg-amber-100 text-amber-700 hover:bg-amber-200 border border-amber-200";
          if (type === "Off")
            return "bg-slate-100 text-slate-400 hover:bg-slate-200 border border-slate-200 decoration-slate-400 line-through decoration-2";
          if (type === "Veg")
        ...
        // Highlight Today
        let containerClass =
          "flex flex-col p-1 md:p-1.5 rounded-xl md:rounded-2xl border transition-all hover:shadow-md h-16 md:h-24 justify-between bg-white";
        if (dateStr === todayStr) {
          containerClass +=
        ...
        pillsDiv.className =
          "flex flex-grow items-center justify-between gap-1"; // Horizontal flex
    ...
    .catch((err) => {
      console.error(err);
      grid.innerHTML =
        "<p class='col-span-7 text-center text-red-500 py-8'>Error loading calendar.</p>";
    });
}
...
function saveBulkLeave() {
  ...
  if (start < todayStr) {
    alert("Cannot mark leave for past dates. Please select a future date.");
    return;
  }
  if (start > end) {
  ...
  db.collection("users")
    .doc(user.uid)
    .update({ futurePreferences: prefs })
    .then(() => {
  ...
  db.collection("attendance")
    ...
    .then((querySnapshot) => {
      if (querySnapshot.empty) {
        historyList.innerHTML = `<p class="text-slate-400 text-sm text-center italic">No meals scanned yet.</p>`;
        return;
      }
...
function initOwnerDashboard(owner) {
  ...
  if (membersLoading)
    membersLoading.innerHTML =
      "<p class='text-center text-slate-400 py-8'>Loading members...</p>";

  // Filter only by MESS ID (avoid composite index issues)
  db.collection("users")
    .where("currentMessId", "==", owner.uid)
    .onSnapshot(
      (snapshot) => {
        globalOwnerMembers = [];
        snapshot.forEach((doc) => {
          ...
          console.log(
            `Found User: ${data.name || data.email} (Role: ${data.role})`,
          );
        ...
        console.log(
          "Member Snapshot: Loaded",
          globalOwnerMembers.length,
          "members",
        );
        ...
        if (globalOwnerMembers.length === 0) {
          if (membersLoading)
            membersLoading.innerHTML =
              "<p class='text-center text-slate-400 py-8'>No members found linked to this mess.</p>";
        }
        if (currentStatsDate) {
      ...
      (err) => {
        ...
        if (membersLoading)
          membersLoading.innerHTML =
            "<p class='text-center text-red-400 py-8'>Error loading members.</p>";
      },
    );
...
function recalculateOwnerStats() {
  ...
  const defMenu = (owner.weeklyMenu && owner.weeklyMenu[dayName]) || {
    lunch: "Veg",
    dinner: "Veg",
  };
  ...
  const getEffectiveMeal = (prefType, mealKey, menu, defaultType) => {
    ...
    if (menu && menu[mealKey]) {
      if (prefType === "NonVeg" && !menu[mealKey].nonveg.available)
        return "Veg";
    }
  ...
  members.forEach((m) => {
    let pref = m.futurePreferences
      ? m.futurePreferences[currentStatsDate]
      : null;
    if (typeof pref === "string") pref = { lunch: pref, dinner: pref };
    pref = pref || {};

    // Use Preference, fallback to Weekly Default
    let lType = pref.lunch || defLunch;
    let dType = pref.dinner || defDinner;
    ...
    if (counts[lType]) counts[lType].L++;
    else counts.Veg.L++;
    if (counts[dType]) counts[dType].D++;
    else counts.Veg.D++;
  });
  ...
  // Detailed Counts
  setTxt("count-veg-l", counts.Veg.L);
  setTxt("count-veg-d", counts.Veg.D);
  setTxt("count-nonveg-l", counts.NonVeg.L);
  setTxt("count-nonveg-d", counts.NonVeg.D);
  setTxt("count-fast-l", counts.Fast.L);
  setTxt("count-fast-d", counts.Fast.D);
  setTxt("count-off-l", counts.Off.L);
  setTxt("count-off-d", counts.Off.D);
...
function renderOwnerStats() {
  ...
  // 2. Update Titles
  if (document.getElementById("label-expecting"))
    document.getElementById("label-expecting").innerText =
      `Expecting ${labelSuffix}`;
  if (document.getElementById("label-preferences"))
    document.getElementById("label-preferences").innerText =
      `🍽️ Meal Preferences (${labelSuffix})`;
  if (document.getElementById("label-leave-title"))
    document.getElementById("label-leave-title").innerText =
      `On Leave (${labelSuffix})`;

  // 3. Update Expecting
  if (document.getElementById("stat-expected-lunch"))
    document.getElementById("stat-expected-lunch").innerText = data.expected.L;
  if (document.getElementById("stat-expected-dinner"))
    document.getElementById("stat-expected-dinner").innerText = data.expected.D;
  ...
  set("count-veg-l", c.Veg.L);
  set("count-veg-d", c.Veg.D);
  set("count-nonveg-l", c.NonVeg.L);
  set("count-nonveg-d", c.NonVeg.D);
  set("count-fast-l", c.Fast.L);
  set("count-fast-d", c.Fast.D);
  set("count-off-l", c.Off.L);
  set("count-off-d", c.Off.D);

  // 5. Update Leave Total
  if (document.getElementById("stat-leave"))
    document.getElementById("stat-leave").innerText = data.leave;
}
...
function renderMemberList(members, today) {
  ...
  if (!container) return;
  if (members.length === 0) {
    container.innerHTML = `<p class="p-4 text-center text-slate-400">No members.</p>`;
    return;
  }
             ...
             <tbody class="divide-y divide-slate-100">
                ${members
                  .map((m) => {
                    ...
                    const getBadge = (t) => {
                      if (t === "Veg")
                        return `<span class="text-green-600 font-bold text-xs bg-green-50 px-1 rounded">V</span>`;
                      if (t === "NonVeg")
                        return `<span class="text-red-600 font-bold text-xs bg-red-50 px-1 rounded">NV</span>`;
                      if (t === "Fast")
                        return `<span class="text-yellow-600 font-bold text-xs bg-yellow-50 px-1 rounded">F</span>`;
                      if (t === "Off")
                        return `<span class="text-slate-400 font-bold text-xs bg-slate-100 px-1 rounded line-through">Off</span>`;
                      return t;
                    };
...
function loadLiveFeed() {
  ...
  db.collection("attendance")
    ...
    .onSnapshot(
      (snapshot) => {
        if (snapshot.empty) {
          feed.innerHTML = `<p class="text-slate-400 text-sm text-center">No scans today.</p>`;
          return;
        }
...
function runCamera(id, isRetry = false) {
  if (isScanning) return; // Prevent double start
  isScanning = true;

  html5QrCode
    .start(id, { fps: 10, qrbox: { width: 250, height: 250 } }, (decoded) => {
      // SUCCESS
      stopScanner().then(() => {
        handleScan(decoded);
      });
    ...
    .catch((err) => {
      ...
      const isNotReadable = errStr.includes("NotReadableError");

      // Auto-fallback: If the first try fails (usually back camera), try the first camera (front/default)
      if (
        isNotReadable &&
        !isRetry &&
        cameras &&
        ...
        console.log(
          "Auto-falling back to another camera due to NotReadableError...",
        );
        currentCameraId = cameras[0].id; // Fallback to 0
        setTimeout(() => runCamera(currentCameraId, true), 300);
        return;
      }

      if (isNotReadable) {
        alert(
          "Camera is busy or not accessible. \n\nPlease ensure no other app/tab is using it, and that permissions are granted. Press OK to close.",
        );
        stopScanner();
      } else {
        alert("Camera Start Failed: " + errStr);
        stopScanner();
      }
    });
}
...
function handleScan(scannedUid) {
  const now = new Date();
  ...
  if (hour >= 11 && hour < 16) meal = "Lunch";
    ...
    if (
      confirm(
        `⚠️ It is ${hour}:00. Strictly speaking, it's not meal time.\n\nMark as LUNCH? (Cancel for Dinner)`,
      )
    ) {
      meal = "Lunch";
    } else {
  ...
  // Determine who is scanning who
  if (currentUser.role === "owner") {
    ownerUid = currentUser.uid;
    memberUid = scannedUid;
  } else {
    memberUid = currentUser.uid;
    ownerUid = scannedUid;

    if (currentUser.currentMessId !== ownerUid) {
      alert(`❌ ACCESS DENIED\n\nYou are not a member of this canteen.`);
      return;
    }
  }
  ...
  db.collection("users")
    ...
    .then((doc) => {
      ...
      if (member.currentMessId !== ownerUid) {
        alert(
          `❌ ACCESS DENIED\n\n${member.name} is not a member of this mess.`,
        );
        return;
      }

      // 2. Check Leave
      let pref = member.futurePreferences
        ? member.futurePreferences[today]
        : null;
      if (typeof pref === "string") pref = { lunch: pref, dinner: pref };
      pref = pref || {};

      const mealPref = pref[meal.toLowerCase()];
      const isOff =
        mealPref === "Off" ||
        (member.holidays && member.holidays.includes(today));

      if (isOff) {
        const msg =
          currentUser.role === "owner"
            ? `⚠️ WARNING: ${member.name} is marked as ON LEAVE for ${meal.toUpperCase()}.\n\nAllow entry anyway?`
            : `⚠️ WARNING: You marked yourself ON LEAVE for ${meal.toUpperCase()}.\n\nProceed with scan?`;

        if (!confirm(msg)) return;
      }

      // 3. Mark Attendance
      // Using Old ID format: uid_today_meal
      const attId = `${memberUid}_${today}_${meal}`;
...
function renderMenuTable(datesToRender, dataMap) {
  ...
  datesToRender.forEach((d) => {
    ...
    const getBadges = (mealType) => {
      ...
      if (o)
        badges.push(
          `<span class="px-1.5 py-0.5 rounded text-[10px] font-bold bg-slate-100 text-slate-500 line-through">Off</span>`,
        );
      else {
        if (v)
          badges.push(
            `<span class="px-1.5 py-0.5 rounded text-[10px] font-bold bg-green-100 text-green-700">Veg</span>`,
          );
        if (n)
          badges.push(
            `<span class="px-1.5 py-0.5 rounded text-[10px] font-bold bg-red-100 text-red-700">NV</span>`,
          );
        if (f)
          badges.push(
            `<span class="px-1.5 py-0.5 rounded text-[10px] font-bold bg-yellow-100 text-yellow-700">Fast</span>`,
          );
      }
...
function renderMenuView(skipFetch = false) {
  ...
  container.innerHTML =
    "<p class='text-center text-slate-400 py-4'>Loading...</p>";
  ...
  Promise.all(requests)
    ...
    .catch((err) => {
      console.error(err);
      container.innerHTML =
        "<p class='text-center text-red-500 py-4'>Error loading menu</p>";
    });
}
...
function openQuickEdit(dateStr, mealType, event) {
  ...
  // 2. Set Title
  document.getElementById("quick-edit-title").innerText =
    `Edit ${mealType} (${dateStr})`;
...
function showToast(msg) {
  ...
  if (!toast) {
    ...
    toast.id = "toast-msg";
    toast.className =
      "fixed bottom-5 left-1/2 transform -translate-x-1/2 bg-slate-800 text-white px-6 py-3 rounded-full shadow-lg z-[100] transition-opacity duration-300 opacity-0 pointer-events-none";
...
function installApp() {
  if (deferredPrompt) {
    ...
    alert(
      "To install the app, click the 'Share' or 'Menu' (three dots) button in your browser and select 'Add to Home Screen' or 'Install App'. Note: You may need to access this site using HTTPS.",
    );
  }
}
